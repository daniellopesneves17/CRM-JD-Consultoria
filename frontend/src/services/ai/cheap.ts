...
export async function analyzeSentiment(messageContent: string) {
  ...
  return parseStructured(await respond({ model: CHEAP_MODEL, instructions: SENTIMENT_PROMPT, input: messageContent, promptType: "sentiment" }), schema);
}
...
export async function detectIntent(messageContent: string) {
  ...
  return parseStructured(await respond({ model: CHEAP_MODEL, instructions: INTENT_PROMPT, input: messageContent, promptType: "intent" }), schema);
}
