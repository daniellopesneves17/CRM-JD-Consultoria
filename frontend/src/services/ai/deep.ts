...
const DEEP_MODEL = process.env.OPENAI_DEEP_MODEL ?? "o3";
const FALLBACK_MODEL = process.env.OPENAI_DEEP_FALLBACK_MODEL ?? "gpt-4o";

async function deepJson<T>(instructions: string, input: unknown, schema: z.ZodType<T>, promptType: string, leadId?: string): Promise<T> {
  try {
    return parseStructured(await respond({ model: DEEP_MODEL, instructions, input: JSON.stringify(input), reasoning: "low", promptType, leadId }), schema);
  } catch (error) {
    if (DEEP_MODEL === FALLBACK_MODEL) throw error;
    return parseStructured(await respond({ model: FALLBACK_MODEL, instructions, input: JSON.stringify(input), reasoning: "low", promptType, leadId }), schema);
  }
}
...
export function scoreLeadConversation(params: { leadId: string; allMessages: Array<{ sender: string; content: string; sentAt: Date }>; leadProfile: { name: string; livesCount: number; stage: string; estimatedValue?: number } }) {
  return deepJson(SCORE_PROMPT, params, scoreSchema, "score", params.leadId);
}
...
export function summarizeConversation(params: { conversationId: string; messages: Array<{ sender: string; content: string }> }) {
  return deepJson(SUMMARY_PROMPT, params, summarySchema, "summary");
}
...
export function qualifyLead(params: { conversationHistory: Array<{ sender: string; content: string }>; leadProfile: { name: string; livesCount: number } }) {
  return deepJson(QUALIFICATION_PROMPT, params, qualificationSchema, "qualification");
}
...
export function suggestReactivationStrategy(params: { leadProfile: { name: string; stage: string; lostReason?: string; notes?: string }; lastMessages: Array<{ sender: string; content: string }> }) {
  return deepJson(`${PERSONA_BASE}\n${REACTIVATION_PROMPT}\nResponda apenas JSON com message, approach e bestTime.`, params, strategySchema, "reactivation");
}
