...
export default function DashboardPage(){
 ...
 return <><div className="flex items-end justify-between"><div><p className="label">Visão geral</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Dashboard</h1><p className="mt-1 text-sm text-slate-500">Ritmo comercial, prioridades e resultados da operação.</p></div><p className="hidden capitalize text-sm text-slate-500 sm:block">{today}</p></div>
 <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><KpiCard label="Leads ativos" value={String(metrics.leads)} icon={Users}/><KpiCard label="Conversas hoje" value={String(metrics.conversationsToday)} icon={MessageSquare}/><KpiCard label="Propostas abertas" value={String(metrics.proposalsOpen)} icon={Clock3}/><KpiCard label="Receita do mês" value={money(metrics.revenue)} icon={BadgeDollarSign}/></div>
