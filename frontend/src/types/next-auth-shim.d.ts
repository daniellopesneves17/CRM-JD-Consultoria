...
declare module "next-auth" {
  ...
  export interface User { id: string; name?: string | null; email?: string | null; image?: string | null; role: "ADMIN" | "CORRETOR"; crmEnabled: boolean; sessionVersion: number }
  export interface Session { user: User; expires: string }
  export interface JWT { sub?: string; role?: "ADMIN" | "CORRETOR"; crmEnabled?: boolean; sessionVersion?: number; [key: string]: unknown }
  export type NextAuthConfig = {
    trustHost?: boolean; secret?: string; session?: { strategy: "jwt"; maxAge?: number }; pages?: { signIn?: string };
    providers: unknown[];
    callbacks?: {
      jwt?: (args: { token: JWT; user?: User; trigger?: "signIn" | "signUp" | "update"; session?: Record<string, unknown> }) => JWT | Promise<JWT>;
      session?: (args: { session: Session; token: JWT }) => Session | Promise<Session>;
    };
  };
  export default function NextAuth(config: NextAuthConfig | ((request?: Request) => NextAuthConfig | Promise<NextAuthConfig>)): {
...
declare module "next-auth/react" {
  ...
  export function useSession(): { data: Session | null; status: "loading" | "authenticated" | "unauthenticated"; update(data?: Record<string, unknown>): Promise<Session | null> };
